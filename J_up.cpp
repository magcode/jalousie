...
const char msgENT_STEP1_1 [] PROGMEM = "-driving up till self hold";
const char msgENT_STEP1_2 [] PROGMEM = "-short drive";
...
const char msgENT_STEP6 [] PROGMEM = "-release power relay after short drive up";

  
J_up& J_up::begin(int pin_rd, int pin_rp) {
	  const static state_t state_table[] PROGMEM = {
	  /*           ON_ENTER  ON_LOOP    ON_EXIT  EVT_TIMER_SELBSTHALT  EVT_TIMER_SWITCHWAIT  EVT_TIMER_WAITTOSTOP  EVT_TIMER_STOPCOMMAND   EVT_TIMER_SHORTDRIVE  EVT_ON      ELSE */
    /* IDLE  */   ENT_OFF,      -1,        -1,                   -1,                   -1,                   -1,          -1,               -1,                 STEP1,      -1,
	  /* STEP1 */ ENT_STEP1,      -1, EXT_STEP1,                STEP2,                   -1,                   -1,          -1,            STEP6,                    -1,      -1,
	  /* STEP2 */ ENT_STEP2,      -1, EXT_STEP2,                   -1,                   -1,                STEP3,          -1,               -1,                    -1,      -1,
	  /* STEP3 */ ENT_STEP3,      -1, EXT_STEP3,                   -1,                STEP4,                   -1,          -1,               -1,                    -1,      -1,
	  /* STEP4 */ ENT_STEP4,      -1, EXT_STEP4,                   -1,                   -1,                   -1,       STEP5,               -1,                    -1,      -1,
		/* STEP5 */ ENT_STEP5,      -1, EXT_STEP5,                   -1,                 IDLE,                   -1,          -1,               -1,                    -1,      -1,
    /* STEP6 */ ENT_STEP6,      -1,        -1,                   -1,                   -1,                   -1,          -1,             IDLE,                    -1,      -1
	  };
...
int J_up::event(int id) {
  ...
  case EVT_TIMER_SHORTDRIVE:
    return timer_driving_up_short.expired(this);
	}
	return 0;
}
...
void J_up::action(int id) {
    ...
    debug(msgENT_OFF);
		return;
	case ENT_STEP1:
    if (timeup > timeSelbstHalt)
    {
      debug(msgENT_STEP1_1);
      timer_driving_up_short.set(-1);
      timer_selbsthalt.set(timeSelbstHalt);
      digitalWrite(pin_rp, LOW);
    } else {
      debug(msgENT_STEP1_2);
      timer_selbsthalt.set(-1);
      timer_driving_up_short.set(timeup);
      digitalWrite(pin_rp, LOW);
    }
    return;
	case EXT_STEP1:
